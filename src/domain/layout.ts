...
export interface BlockGeometry {
   ...
   * 高度是「补」出来的，不是自然高度。
   *
   * 只有落在被压缩的课间空档里的钟点事件会这样：空档在固定行高下占 0 像素
   * （见 timeAxis.ts 的映射规则 2），事件没有任何属于自己的像素，只能给一个
   * 高度下限，让它至少可见、可点。
   *
   * ⚠️ 这类块在**真实时间上和相邻课程并不重叠**，纯粹是网格没给它留位置。
   * 所以它绝不能和普通块一起分道 —— 否则一个 7 分钟的课间事件会把两侧正常的
   * 课程挤成半宽（见 layoutDay）。渲染上也单独处理（见 EntryBlock 的 floating）。
   */
  floating?: boolean;
}
...
export interface PositionedEntry {
  ...
  collapsed: boolean;
  /**
   * 落在被压缩课间空档里的钟点事件（见 BlockGeometry.floating）。
   *
 ...
 *
 * 浮块是落在被压缩课间空档里的钟点事件（见 BlockGeometry.floating）：它那 20px
 * 高度是补出来的，真实时间上跟两侧课程都不重叠，只是网格没给它留像素。
